...
class Part {
  ...
  async loadSVG() {
    ...
    let svgUrl = new URL(this.config.svg, new URL('tinygo/parts/', document.baseURI));
...
async function loadPartsPanel() {
  ...
  let response = await fetch('tinygo/parts/parts.json');
  ...
  for (let part of json.parts) {
    ...
    let svgPromise = loadSVG(new URL(part.config.svg, new URL('tinygo/parts/', document.baseURI)))
    svgPromise.then((svg) => {
